...
- **پنل پیشرفته** — اگر خواستید خودتان کنترل داشته باشید، یک پنل بازشونده همان سه گزینه‌ای را که Aether واقعاً می‌پرسد در اختیارتان می‌گذارد:
  - **پروتکل**: MASQUE (روی HTTP/3-QUIC، ترافیک را شبیه HTTPS معمولی جا می‌زند)، WireGuard (سبک‌تر و سریع‌تر) یا WARP-in-WARP/gool (دو تونل WireGuard تو در تو، امنیت بیشتر به قیمت کمی افت سرعت)
  - **حالت اسکن**: Turbo، Balanced، Thorough یا Stealth — یعنی این‌که چقدر سریع دنبال مسیر بگردد در برابر این‌که چقدر ترافیکِ کاوش تولید کند
...
- **فرانت‌اند**: React 19 و Tailwind v4، مدیریت وضعیت با Zustand، انیمیشن‌ها با [Motion](https://motion.dev/)، و یک پس‌زمینه‌ی سه‌بعدیِ WebGL با [React Three Fiber](https://r3f.docs.pmnd.rs/) — همه از طریق IPCِ Tauri با بک‌اند Rust حرف می‌زنند.
- **بک‌اند**: Rust، با استفاده از [`portable-pty`](https://docs.rs/portable-pty) که فایل واقعیِ `aether` را داخل یک شبه‌ترمینالِ واقعی اجرا می‌کند (لازم است، چون سؤال‌های تعاملیِ Aether روی یک لوله‌ی ساده یا اصلاً ظاهر نمی‌شوند یا جور دیگری رفتار می‌کنند). یک ریسه در پس‌زمینه خروجیِ Aether را خط‌به‌خط می‌خواند، سه سؤال اولیه‌اش را از روی متنِ سرخطشان تشخیص می‌دهد و طبق تنظیمات شما جوابشان را می‌دهد — و در همان حال هر خط را زنده به پنل لاگِ برنامه هم می‌فرستد.
